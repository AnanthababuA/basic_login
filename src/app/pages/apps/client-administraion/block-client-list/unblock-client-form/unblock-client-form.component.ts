...
@Component({
  selector: 'app-unblock-client-form',
  templateUrl: './unblock-client-form.component.html',
  styleUrls: ['./unblock-client-form.component.scss']
})
export class UnblockClientFormComponent {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<UnblockClientFormComponent>
  ) {
...
params: any
  ...
  submit(){
    ...
    Swal.fire({
      title: 'Do you really want to unblock the client?',
      // text: file.name,
      showDenyButton: true,
      // showCancelButton: true,
      confirmButtonText: 'Unblock Client',
      denyButtonText: `Cancel`,
    }).then((result) => {
      if (result.isConfirmed) {
        // this.deleteUnitApi();
       this.params = {client_id: this.clientInfo.client_id_id, remark:this.deleteClient.value.remark}
        ...
        Swal.fire('Unblocking Client Cancelled', '', 'info');
      }
    });
  ...
  blockClientAPI(params: any) {
    this.spinner.show();
    // this.unitType = '';
    this.common.unBlockClient(params).subscribe((res: any) => {
