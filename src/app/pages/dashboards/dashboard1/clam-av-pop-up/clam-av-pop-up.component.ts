...
@Component({
  selector: 'app-clam-av-pop-up',
  templateUrl: './clam-av-pop-up.component.html',
  styleUrls: ['./clam-av-pop-up.component.scss'],
  standalone: true,
  imports: [DataTablesModule, CommonModule,  MatButtonModule,MatDialogModule ]
})
export class ClamAvPopUpComponent {
  ...
  clamAvAPI() {
    this.spinner.show();
    this.clamAvData = '';
    this.common.dashboardClamAvUpdateDetails().subscribe(
      (res: any) => {
        if (res.api_status === true) {
          this.spinner.hide();
          this.clamAvData = res.data;
